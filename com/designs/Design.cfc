<cfcomponent displayname="Design" hint="Methods for handling Designs and Design Specs" output="false">
	...
	<cffunction access="remote" name="list" hint="returns xml for all designs matching criteria">
		...
		<cfif appMode eq "manager">
			<cfset templ = structNew() />
			<cfset templ.i = "manager_placeholder-logo" />
			<cfset templ.isphoto = 0 />
			<cfset tempembl = structNew() />
			<cfset tempembl.i = "manager_placeholder-emblogo" />
			<cfset tempembl.isphoto = 0 />
			<cfset tempvcl = structNew() />
			<cfset tempvcl.i = "manager_placeholder-vclogo" />
			<cfset tempvcl.isphoto = 0 />
			<cfset tempm = structNew() />
			<cfset tempm.i = "manager_placeholder-mascot" />
			<cfset tempm.isphoto = 0 />
			<cfset tempembm = structNew() />
			<cfset tempembm.i = "manager_placeholder-embmascot" />
			<cfset tempembm.isphoto = 0 />
			<cfset tempvcm = structNew() />
			<cfset tempvcm.i = "manager_placeholder-vcmascot" />
			<cfset tempvcm.isphoto = 0 />
			<cfset arrayAppend(logoImages, templ) />
			<cfset arrayAppend(embLogoImages, tempembl) />
			<cfset arrayAppend(vcLogoImages, tempvcl) />
			<cfset arrayAppend(mascotImages, tempm) />
			<cfset arrayAppend(embMascotImages, tempembm) />
			<cfset arrayAppend(vcMascotImages, tempvcm) />
			<cfif activityId eq -2> 	<!--- get logo designs only --->
				<cfset doLogo = true />
				<!--- <cfset added = ArrayAppend(logoImages, "manager_placeholder-logo") /> --->
				<cfset doEmbLogo = true />
				<!--- <cfset added = ArrayAppend(embLogoImages, "manager_placeholder-emblogo") /> --->
				<cfset doVcLogo = true />
				<!--- <cfset added = ArrayAppend(vcLogoImages, "manager_placeholder-vclogo") /> --->
				<cfquery name="getDesigns" dataSource="#dsn#">
					SELECT d.*,NULL activity_id,NULL activity,3 orderHelper,'Logo' designGroup,NEWID() randomHelper
					FROM [Design] d WITH (NOLOCK)
					<cfif designTypeId eq 0>
					WHERE d.isMascotDesign = 0
					...
					<cfif designStatus eq -2>
						AND d.isActive = 2
					<cfelseif designStatus eq -1 or designStatus eq 1>
						AND d.isActive = 1
					<cfelseif designStatus eq 0>
						AND d.isActive = 0
					</cfif>
					ORDER BY d.design_id
				</cfquery>
			<cfelseif activityId eq -1> 	<!--- get mascot designs only --->
				<cfset doMascot = true />
				<!--- <cfset added = ArrayAppend(mascotImages, "manager_placeholder-mascot") /> --->
				<cfset doEmbMascot = true />
				<!--- <cfset added = ArrayAppend(embMascotImages, "manager_placeholder-embmascot") /> --->
				<cfset doVcMascot = true />
				<!--- <cfset added = ArrayAppend(vcMascotImages, "manager_placeholder-vcmascot") /> --->
				<cfquery name="getDesigns" dataSource="#dsn#">
					SELECT d.*,NULL activity_id,NULL activity,3 orderHelper,'Mascot' designGroup,NEWID() randomHelper
					FROM [Design] d WITH (NOLOCK)
					<cfif designTypeId eq 0>
					WHERE d.isMascotDesign = 1
			...
			<cfelseif activityId eq 0 or activityId eq "" or activityId eq -99> 	<!--- get general designs (no activity) only --->
				<cfquery name="getDesigns" dataSource="#dsn#">
					SELECT d.*,NULL activity_id,NULL activity,5 orderHelper,'noActivity' designGroup,NEWID() randomHelper
					FROM [Design] d WITH (NOLOCK)
					<cfif designTypeId eq 0>
					WHERE d.isMascotDesign = 0
						AND d.isLogoDesign = 0
			...
			<cfelse> 	<!--- get specified activity designs only --->
				<cfquery name="getDesigns" dataSource="#dsn#">
					SELECT 		d.*,a.activity_id,a.name activity,5 orderHelper,'activity' designGroup,NEWID() randomHelper
					FROM 		[Design] d WITH (NOLOCK)
